...
def load_train_X(run_id: str) -> pd.DataFrame:
    ...
    train_X = pd.read_csv(
        MlflowUtils.get_artifact_path(run_id, ARTIFACT_TRAIN_X))
    return train_X
...
def load_train_y(run_id: str) -> pd.DataFrame:
    _assert_ops_type(run_id)
    train_y = pd.read_csv(
        MlflowUtils.get_artifact_path(run_id, ARTIFACT_TRAIN_Y))
    return train_y
...
def load_test_X(run_id: str) -> pd.DataFrame:
    _assert_ops_type(run_id)
    test_X = pd.read_csv(
        MlflowUtils.get_artifact_path(run_id, ARTIFACT_TEST_X))
    return test_X
...
def load_test_y(run_id: str) -> pd.DataFrame:
    _assert_ops_type(run_id)
    test_y = pd.read_csv(
        MlflowUtils.get_artifact_path(run_id, ARTIFACT_TEST_Y))
    return test_y
